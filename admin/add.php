...
if (!empty($_POST)) {
    ...
    if (
        ...
        if (empty($upload['error'])) {
            ...
            $query = $db->prepare('INSERT INTO posts (title, content, cover, created_at, user_id, categorie_id ) VALUES (:title, :content, :cover,  NOW() , 1 , :categorie_id)' );

            $query->bindValue(':title', $title);
            $query->bindValue(':content', $content);
            $query->bindValue(':cover', $fileName);
            $query->bindValue(':categorie_id', $category, PDO::PARAM_INT);
            $query->execute();
